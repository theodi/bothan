function applyNumber(title, date, number, element, currency) {
  ...
  num = scaleNumber(number, currency)
...
function number(json, title, element, currency) {
  if (json.values) {
  ...
  applyNumber(title, date, number, element, currency)
}
