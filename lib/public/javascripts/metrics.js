...
function updateEmbedCode(params, from, to) {
  embed = $('#embed textarea')
  iframe = $('#iframe_embed iframe')

  uri = URI(iframe.attr('src'))
  ...
  embed.val(embedCode(url))
  iframe.attr('src', url)
  history.pushState({}, document.title, uri.removeQuery(['layout', 'plotly_modebar']).toString())
}
