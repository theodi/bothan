...
function getPoints(metrics, extras) {
  return {
    ...
    text: metrics.map(function(metric) {
      return [extras.prefix, metric['value'], extras.suffix].join('')
    })
  }
}
...
function updatePage(params, from, to, uri) {
  uri = typeof a !== 'undefined' ? url : URI($('#iframe_embed iframe').attr('src'));
