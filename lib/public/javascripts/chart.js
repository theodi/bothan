function graph(title, json, textcolour, boxcolour, plotly_modebar, datatype, dateFormat) {
  ...
  if (typeof(points['y'][0]) == 'object') {
    ...
    var data = singlePoint(points, textcolour)
  }

  var layout = getLayout(title, textcolour, boxcolour)

  var last = json.values[json.values.length - 1]
  var lastUpdated = moment(last['time']).format(dateFormat)
