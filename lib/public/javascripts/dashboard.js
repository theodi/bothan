...
function growCol(td, span) {
  var table = td.parents('table')
  var row = td.closest("tr")
  var index = td.index()
  var old_span = (typeof td.attr('colspan') === 'undefined') ? 1 : Number(col.attr('colspan'))
  ...
  if (span > old_span) {
    ...
    if (num < numCol(table)) {
      for (var count=0; count < (numCol(table) - num) ; count++) { row.append('<td></td>') };
    }
